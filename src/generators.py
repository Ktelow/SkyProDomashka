...
def filter_by_currency(transactions: list[dict[str, Any]], currency_set: str) -> Generator:
    """Функция, которая выводит транзакции по заданной валюте"""
    for transaction in transactions:
        if transaction["operationAmount"]["currency"]["code"] == currency_set:
            yield transaction


def transaction_descriptions(transactions: list[dict[str, Any]]) -> Generator:
    """Генератор, который выводит описание каждой операции в списке"""
    for transaction in transactions:
